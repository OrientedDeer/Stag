...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
	...
	const planSubLinks = [
        { path: "/plan/assumptions", label: "Assumptions" },
        { path: "/plan/allocation", label: "Allocation" },
        { path: "/plan/withdrawal", label: "Withdrawal" },
    ];
	...
	// Material "tune" sliders — plan inputs
	const planIcon = (
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="white" viewBox="0 -960 960 960">
	...
	// Trending-up chart — projection outputs
		...
		<>
			...
			`}>
				<nav className="flex flex-col gap-1 pt-14 md:pt-0 flex-1">
					...
					<Link
						className={`${link} ${pathname.startsWith("/projection") && active} ${isOpen ? "pointer-events-none" : ""}`}
						to="/projection"
						aria-current={pathname.startsWith("/projection") ? "page" : undefined}
						onClick={handleLinkClick}
					>
						<span className={`flex items-center gap-2 overflow-hidden whitespace-nowrap transition-all duration-300 ${isOpen ? "w-0 opacity-0" : "w-auto opacity-100"}`}>
							{projectionIcon}
