...
export default function TopBar({setIsOpen, title}: TopBarProps ) {
	return (
		<div
			className={`w-full text-white h-10 bg-surface-raised transition-all duration-300 flex flex-row items-center pl-2`}
		>
			<button
                onClick={() => setIsOpen(prev => !prev)}
                aria-label="Toggle navigation menu"
                className="flex items-center gap-2 p-2 rounded-lg hover:bg-surface-input transition-colors"
			>
