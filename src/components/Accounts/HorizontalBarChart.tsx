...
import { 
    ...
    PropertyAccount, 
    DebtAccount 
} from './models'; // Assume models are imported from where your classes are defined

// --- 1. New Structure Mappings (Replacing external constants) ---

// Define Categories based on the Class names (used for grouping and labels)
const ACCOUNT_CATEGORIES = ['Saved', 'Invested', 'Property', 'Debt'] as const;
type AccountCategory = typeof ACCOUNT_CATEGORIES[number];

// Map Class Constructors to their Category name
const CLASS_TO_CATEGORY: Record<string, AccountCategory> = {
...
const CATEGORY_PALETTES: Record<AccountCategory, string[]> = {
...
type HorizontalBarChartProps = {
    type: string,
    // The component now accepts the array of AnyAccount (our union type)
...
export default function HorizontalBarChart({ type, accountList }: HorizontalBarChartProps) {
    ...
    // Original helper function remains the same
    ...
    const { chartData, totalAssets } = useMemo(() => {
        // Initialize groups using the new AccountCategory type
        const grouped: Record<AccountCategory, AnyAccount[]> = ACCOUNT_CATEGORIES
            .reduce((acc, c) => ({ ...acc, [c]: [] }), {} as Record<AccountCategory, AnyAccount[]>);

        for (const acc of accountList) {
            // CRITICAL CHANGE: Determine category based on the constructor name
    ...
    return (
        <div className="mb-2">
            <div className='flex justify-center text-white'>
                {/* Updated display to show the type (e.g., 'Total Assets') and the calculated total */}
                {type} ${totalAssets.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
            <div className="w-full h-3 flex rounded-lg overflow-hidden mt-1">
                {chartData.map(seg => (
                    <div
                        key={`${seg.category}-${seg.account}`} // Use category in key for uniqueness
