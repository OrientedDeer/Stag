...
export default function CloudBackupPanel({ isOpen, onClose }: CloudBackupPanelProps) {
    ...
    const sectionHeader = "text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2";
    const actionButton = "w-full flex items-center gap-2 px-3 py-2 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-lg text-sm text-white transition-colors";
    ...
    return (
        <>
            ...
            <div
                ref={panelRef}
                className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-96 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl overflow-hidden"
            >
                {/* Header */}
                    ...
                    {lastError && (
                        <div className="bg-red-900/20 border border-red-800 rounded-lg p-2">
                            <div className="flex justify-between items-start">
                                <p className="text-red-400 text-sm flex-1">{lastError}</p>
                        ...
                        <div className="space-y-2">
                            ...
                            {!confirmDeleteAll ? (
                                ...
                                <div className="bg-red-900/20 border border-red-800 rounded-lg p-3">
                                    <p className="text-red-400 text-sm mb-2">Permanently delete all local data?</p>
                    ...
                    <div>
                        ...
                        {!enabled ? (
                            <div className="bg-gray-800 rounded-lg p-3">
                                <p className="text-gray-500 text-xs">Cloud backup is not configured. Set environment variables to enable.</p>
                            </div>
                        ) : checkingAuth ? (
                            ...
                            <div className="space-y-2">
                                <div className="bg-blue-900/20 border border-blue-700/50 rounded-lg p-2">
                                    <p className="text-blue-400 text-xs">Encrypted on your device before upload. The server never sees your data.</p>
                                </div>
                                <button
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-2 bg-gray-800 rounded-lg p-2">
                                    <div className="w-6 h-6 bg-green-600/20 rounded-full flex items-center justify-center shrink-0">
                                        <svg className="w-3 h-3 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                        </svg>
                                    </div>
                                    <p className="text-white text-xs truncate flex-1">{userEmail}</p>
                                    <button onClick={signOut} className="text-gray-500 hover:text-gray-300 text-xs shrink-0">
                                        Sign out
                                    </button>
                                </div>
                                ...
                                {lastBackupTimestamp && (
                                    <p className="text-gray-500 text-xs px-1">Last backup: {formatTimestamp(lastBackupTimestamp)}</p>
                                )}
                                ...
                                <button
                                    ...
                                    className="w-full px-3 py-2 bg-green-600 hover:bg-green-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2"
                                >
                                    {backupInProgress ? (
                                        <>
                                ...
                                {confirmDelete && (
                                    <div className="bg-red-900/20 border border-red-800 rounded-lg p-3">
                                        <p className="text-red-400 text-sm mb-2">Permanently delete your cloud backup?</p>
                                        <div className="flex gap-2">
                                            <button
