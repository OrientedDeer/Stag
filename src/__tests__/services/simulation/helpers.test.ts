...
import {
    ...
    estimateTraditionalWithdrawalForExpenses,
    ACAOptions,
} from '../../../services/simulation/helpers';
import { InvestedAccount, SavedAccount, ESPPAccount, AnyAccount } from '../../../components/Objects/Accounts/models';
...
describe('helpers', () => {
    ...
    describe('estimateTraditionalWithdrawalForExpenses', () => {
        // Helper to create withdrawal strategy
        function createStrategy(accountIds: string[]): { accountId: string }[] {
            return accountIds.map(id => ({ accountId: id }));
        }

        describe('no deficit scenarios', () => {
            it('should return 0 when preliminaryCash is positive', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 100000)
                ];
                const strategy = createStrategy(['test-Traditional 401k']);

                const result = estimateTraditionalWithdrawalForExpenses(
                    5000, // positive cash
                    accounts,
                    strategy
                );
            ...
            it('should return 0 when preliminaryCash is zero', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 100000)
                ];
                const strategy = createStrategy(['test-Traditional 401k']);
        ...
        describe('basic deficit coverage', () => {
            it('should estimate Traditional withdrawal for deficit', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 100000)
                ];
                const strategy = createStrategy(['test-Traditional 401k']);
                ...
                expect(result).toBeGreaterThan(10000); // Should be grossed up for taxes
            });

            it('should use 25% estimated tax rate for gross-up', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 100000)
                ];
                const strategy = createStrategy(['test-Traditional 401k']);
                ...
                // With 25% tax rate, gross needed = deficit / (1 - 0.25) = deficit / 0.75
                const expectedGross = deficit / 0.75;
                expect(result).toBeCloseTo(expectedGross, 0);
            });

            it('should cap at available Traditional balance', () => {
                const smallBalance = 5000;
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', smallBalance)
                ];
                const strategy = createStrategy(['test-Traditional 401k']);
                ...
                // Should be capped at account balance
                expect(result).toBe(smallBalance);
            });
        });

        describe('withdrawal strategy order', () => {
            it('should follow withdrawal strategy order', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 50000),
                    createInvestedAccount('Traditional IRA', 100000)
                ];
                // Traditional 401k is first in strategy
                const strategy = createStrategy(['test-Traditional 401k', 'test-Traditional IRA']);
                ...
                // Gross needed for $30k = $40k, should come from 401k first
                expect(result).toBeGreaterThan(0);
                expect(result).toBeLessThanOrEqual(50000); // Should fit in first account
            });

            it('should skip non-Traditional accounts in strategy', () => {
                // The function walks strategy order. Non-Traditional accounts
                // reduce the remaining deficit but don't contribute to the
                // Traditional withdrawal estimate. We need a deficit larger
                // than what non-Traditional can cover.
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Roth IRA', 5000), // Can only cover $5k
                    createInvestedAccount('Traditional 401k', 50000)
                ];
                const strategy = createStrategy(['test-Roth IRA', 'test-Traditional 401k']);

                const result = estimateTraditionalWithdrawalForExpenses(
                    -20000, // $20k deficit - Roth covers $5k, Traditional needed for rest
                    accounts,
                    strategy
                );

                // Roth covers $5k, remaining $15k needs Traditional
                // Gross for $15k = $20k (at 25% tax rate)
                const remainingDeficit = 20000 - 5000;
                const expectedTraditional = remainingDeficit / 0.75;
                expect(result).toBeCloseTo(expectedTraditional, 0);
            });

            it('should handle accounts not in strategy', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 100000)
                ];
                // Empty strategy
                const strategy: { accountId: string }[] = [];
                ...
                // No accounts in strategy, should return 0
                expect(result).toBe(0);
            });
        });

        describe('bracket headroom capping', () => {
            it('should cap at bracket headroom when provided', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 100000)
                ];
                const strategy = createStrategy(['test-Traditional 401k']);

                const bracketHeadroom = 5000; // Only $5k of bracket space
                const result = estimateTraditionalWithdrawalForExpenses(
                    -20000, // $20k deficit (would need ~$26.7k gross)
                    accounts,
                    strategy,
                    bracketHeadroom
                );

                // Should be capped at bracket headroom
                expect(result).toBe(bracketHeadroom);
            });

            it('should stop estimating Traditional when bracket is full', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 100000),
                    createInvestedAccount('Traditional IRA', 100000)
                ];
                ...
                // Should stop at bracket headroom even with more accounts available
                expect(result).toBe(bracketHeadroom);
            });

            it('should work without bracket headroom (undefined)', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 100000)
                ];
                const strategy = createStrategy(['test-Traditional 401k']);
                ...
                const result = estimateTraditionalWithdrawalForExpenses(
                    ...
                    accounts,
                    strategy,
                    undefined // No bracket headroom cap
                );

                // Should estimate full amount needed
                const expectedGross = 10000 / 0.75;
                expect(result).toBeCloseTo(expectedGross, 0);
            });
        });

        describe('multi-account scenarios', () => {
            it('should walk through multiple Traditional accounts', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 10000), // Small balance
                    createInvestedAccount('Traditional IRA', 100000) // Larger balance
                ];
                const strategy = createStrategy(['test-Traditional 401k', 'test-Traditional IRA']);

                // Need more than first account can provide
                const deficit = 30000; // Gross needed ~$40k
                const result = estimateTraditionalWithdrawalForExpenses(
                    -deficit,
                    accounts,
                    strategy
                ...
                // Should use first account ($10k) + some from second
                expect(result).toBeGreaterThan(10000);
                expect(result).toBeCloseTo(deficit / 0.75, 0);
            });

            it('should handle mixed account types in strategy', () => {
                // Make non-Traditional accounts small so deficit reaches Traditional
                const accounts: AnyAccount[] = [
                    new SavedAccount('savings', 'Savings', 3000, 4),
                    createInvestedAccount('Roth IRA', 2000),
                    createInvestedAccount('Traditional 401k', 100000),
                    createInvestedAccount('Brokerage', 5000)
                ];
                const strategy = createStrategy([
                    'savings',
                    'test-Roth IRA',
                    'test-Traditional 401k',
                    'test-Brokerage'
                ]);
                ...
                // Savings covers $3k, Roth covers $2k, then Traditional
                // Remaining after non-Traditional: $15k needs Traditional gross-up
                // Note: Brokerage comes AFTER Traditional in strategy, so not used yet
                const remainingAfterSavingsAndRoth = 20000 - 3000 - 2000;
                const expectedTraditional = remainingAfterSavingsAndRoth / 0.75;
                expect(result).toBeCloseTo(expectedTraditional, 0);
            });

            it('should track remaining deficit across accounts', () => {
                // Set up scenario where non-Traditional covers part of deficit
                const accounts: AnyAccount[] = [
                    new SavedAccount('savings', 'Savings', 5000, 4), // Covers $5k
                    createInvestedAccount('Traditional 401k', 100000)
                ];
                const strategy = createStrategy(['savings', 'test-Traditional 401k']);

                const totalDeficit = 20000;
                const result = estimateTraditionalWithdrawalForExpenses(
                    -totalDeficit,
                    accounts,
                    strategy
                ...
                // Savings covers $5k, remaining $15k needs Traditional
                // Gross for $15k = $20k
                const remainingDeficit = totalDeficit - 5000;
                const expectedTraditional = remainingDeficit / 0.75;
                expect(result).toBeCloseTo(expectedTraditional, 0);
            });
        });
        ...
        describe('edge cases', () => {
            ...
            it('should handle account not found in accounts array', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 100000)
                ];
                // Strategy references non-existent account
                const strategy = createStrategy(['non-existent-id']);
            ...
            it('should handle account with zero balance', () => {
                const accounts: AnyAccount[] = [
                    createInvestedAccount('Traditional 401k', 0) // Zero balance
                ];
                const strategy = createStrategy(['test-Traditional 401k']);
            ...
            it('should handle very small deficit', () => {
                ...
                expect(result).toBeCloseTo(1 / 0.75, 2);
            });
        });
    });
});
