...
describe('TaxService: Gross Up Calculator', () => {
    ...
    // -------------------------------------------------------------------------
    // SCENARIO 6: Early Withdrawal Penalty (10%)
    // -------------------------------------------------------------------------
    it('Scenario 6: 10% Early Withdrawal Penalty in 22% bracket', () => {
        // Setup:
        // Taxable Income: $60,000 (in 22% bracket)
        // Tax Rate: 22%
        // Penalty Rate: 10%
        // Net Needed: $1000
        // Math: Gross = Net / (1 - TaxRate - PenaltyRate) = 1000 / (1 - 0.22 - 0.10) = 1000 / 0.68 = 1470.59

        const result = calculateGrossWithdrawal(
            1000,
            60000,  // Current Fed Income (in 22% bracket: $47,151 - $100,525)
            0,      // Deduction = 0
            60000,  // State Income
            0,      // State Deduction
            createTaxState({ stateResidency: 'Texas' }),
        ...
        expect(result.grossWithdrawn).toBeCloseTo(1470.59, 1);
        expect(result.penalty).toBeCloseTo(147.06, 1); // 10% of gross
        // Verify: grossWithdrawn - totalTax - penalty ≈ netNeeded
        const netResult = result.grossWithdrawn - result.totalTax - result.penalty;
        expect(netResult).toBeCloseTo(1000, 0);
    });

    // -------------------------------------------------------------------------
    // SCENARIO 7: Edge Case - Net Needed = 0
    // -------------------------------------------------------------------------
    it('Scenario 7: Edge Case - netNeeded = 0', () => {
        const result = calculateGrossWithdrawal(
            0,      // Net Needed = 0
            50000,
            0,
        ...
        expect(result.grossWithdrawn).toBeCloseTo(0, 2);
        expect(result.totalTax).toBeCloseTo(0, 2);
        expect(result.penalty).toBe(0);
    });

    // -------------------------------------------------------------------------
    // SCENARIO 8: Verify net = gross - tax - penalty identity
    // -------------------------------------------------------------------------
    it('Scenario 8: Verify grossWithdrawn - totalTax - penalty = netNeeded', () => {
        const testCases = [
            { net: 5000, income: 0, deduction: 14600, penalty: 0 },    // 0% zone
            { net: 10000, income: 30000, deduction: 0, penalty: 0 },  // 12% bracket
            { net: 15000, income: 80000, deduction: 0, penalty: 0 },  // 22% bracket
            { net: 20000, income: 80000, deduction: 0, penalty: 0.10 }, // 22% + 10% penalty
        ];
        ...
        for (const tc of testCases) {
            ...
            const actualNet = result.grossWithdrawn - result.totalTax - result.penalty;
            expect(actualNet).toBeCloseTo(tc.net, 0);
        }
    });
