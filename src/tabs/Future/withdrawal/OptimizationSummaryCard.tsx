        ...
        <div className={`mb-6 bg-green-900/20 border border-green-700/50 rounded-xl p-4 relative ${isRecalculating ? 'opacity-60' : ''}`}>
            ...
            <div className="flex items-center justify-between mb-3">
                ...
                <span className={`px-2 py-0.5 text-xs rounded ${
                    summary.phase === 'BROKERAGE_AVAILABLE' ? 'bg-blue-600' :
                    summary.phase === 'BROKERAGE_TRANSITION' ? 'bg-yellow-600' :
                    summary.phase === 'BROKERAGE_DEPLETED' ? 'bg-orange-600' :
                    'bg-red-600'
                }`}>
                ...
                <div className="space-y-2">
                    ...
                    {summary.maxConversionInPlan > summary.avgConversionPerYear * 1.2 && (
                        <div className="flex justify-between">
                            ...
                            <span className="text-yellow-300">{formatMoney(summary.maxConversionInPlan)}</span>
                        </div>
                    )}
                </div>
            </div>
            ...
            {summary.firstYearConversion > 0 && (
                <div className="mt-4 pt-4 border-t border-green-800/50">
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-lg font-bold text-green-400">{formatMoney(summary.firstYearConversion)}</span>
                    </div>
                </div>
            )}
